...
def _qrc_code_signature() -> str:
    import hashlib

    root = Path(__file__).resolve().parent
    files = [
        root / "plot_model_comparison.py",
        root / "models_classical.py",
        root / "qrc_config.py",
        root / "models" / "photonic_qrc.py",
        root / "models" / "photonic_memory.py",
        root / "models" / "photonic_memory_state.py",
        root / "models" / "quantum_kernel_photmem.py",
    ]
    h = hashlib.sha1()
    for p in files:
        try:
...
def _classical_baseline_code_signature() -> str:
    import hashlib
    ...
    files = [
        root / "plot_model_comparison.py",
        root / "models_classical.py",
        root / "submission_swaptions_new" / "hybrid_qrc_fast.py",
        root / "models" / "photonic_qrc.py",
        root / "models" / "photonic_memory.py",
        root / "models" / "quantum_kernel_photmem.py",
    ]
...
def _load_classical_baseline_cache(path: Path) -> dict[str, Any] | None:
    if not path.exists():
        return None
    try:
        ...
        warnings.warn(f"failed to load classical baseline cache {path}: {exc}", RuntimeWarning)
        return None
    if not isinstance(payload, dict):
        warnings.warn(f"classical baseline cache {path} has invalid format; expected dict.", RuntimeWarning)
        return None
    return payload


def _save_classical_baseline_cache(path: Path, payload: dict[str, Any]) -> None:
...
def _copy_stored_prediction_block(block: Any) -> dict[str, np.ndarray] | None:
    if not isinstance(block, dict):
        return None
    req = ("train", "val", "test", "val_surface")
...
def _load_qrc_config_from_source(
    ...
    *,
    ...
    current_code_sig = _qrc_code_signature()

    def _check_row_code_sig(row: pd.Series) -> None:
        if "qrc_code_signature" not in row.index:
            return
        sig = str(row.get("qrc_code_signature", "")).strip()
        if not sig:
            return
        if sig != current_code_sig and not bool(allow_stale_sweep):
            raise RuntimeError(
                "qrc_load_from sweep artifacts were produced by a different code signature "
                f"(saved={sig}, current={current_code_sig}). "
                "Re-run pqrc_sweep with current code or pass --allow_stale_qrc_sweep."
            )

    def _check_cfg_code_sig(cfg: dict[str, Any]) -> None:
        sig = str(cfg.get("qrc_code_signature", "")).strip()
        if not sig:
            return
        if sig != current_code_sig and not bool(allow_stale_sweep):
            raise RuntimeError(
                "qrc_load_from config was produced by a different code signature "
                f"(saved={sig}, current={current_code_sig}). "
                "Re-run pqrc_sweep with current code or pass --allow_stale_qrc_sweep."
            )
    ...
    if src.is_file():
        ...
        if bool(enforce_optimal):
            ...
            if hist is not None:
                ...
                _check_row_code_sig(best_row)
                best_cfg, best_hash = _row_to_qrc_cfg(best_row)
                _check_cfg_code_sig(best_cfg)
...
def _kernel_gram_summary(gram: np.ndarray) -> dict[str, float]:
    ...
    return {
        "n_rows": float(g.shape[0]),
        "n_cols": float(g.shape[1]),
...
def _line_style_for_model(model_name: str, color_idx: int) -> dict[str, Any]:
    ...
    if m == "hybrid_qrc_fast":
        return {"color": "#1f9d8a", "lw": 2.8, "marker": "X", "ms": 5.0, "alpha": 0.97, "zorder": 7, "ls": "-"}
    if m == "qml_classical_reservoir":
        return {"color": "#9467bd", "lw": 2.5, "marker": "d", "ms": 4.5, "alpha": 0.95, "zorder": 6, "ls": "-"}
    if m == "qml_recursive_classical_reservoir":
        return {"color": "#7f7f7f", "lw": 2.5, "marker": "d", "ms": 4.5, "alpha": 0.95, "zorder": 6, "ls": "--"}
    if m == "qml_pca_lstm":
        return {"color": "#bcbd22", "lw": 2.6, "marker": "o", "ms": 4.6, "alpha": 0.95, "zorder": 6, "ls": "-"}
    if m == "qml_convlstm":
        return {"color": "#17becf", "lw": 2.6, "marker": "o", "ms": 4.6, "alpha": 0.95, "zorder": 6, "ls": "--"}
    ...
    if m == "quantum_kernel_photonic_memory":
        return {"color": "#8c564b", "lw": 2.8, "marker": "D", "ms": 4.8, "alpha": 0.97, "zorder": 7, "ls": "-"}
...
def _build_model_hash(row: dict[str, Any], *, seed: int, level: int, backend: str) -> str:
    ...
    scalar_keys = {
        ...
        "qk_blend_objective",
        "qk_blend_max_weight",
        "qk_blend_grid_points",
        "qk_horizon_mode",
        "qk_target_space",
        "qk_resid_clip",
        "qk_objective_train_rmse",
        "qk_objective_val_rmse",
        "hash_state_features",
        "hash_K_train",
        "qrc_config_hash",
        "dataset_signature",
...
def _run_photonic_coordinate_sweep(
    *,
    ...
    sweep_exact: bool = True,
    sweep_replay_check: bool = True,
) -> dict[str, Any]:
    ...
    def _params_from_cfg(cfg: dict[str, Any]) -> Any:
        ...
        common_kwargs["pseudocount"] = float(cfg.get("pqrc_pseudocount", 0.25))
    ...
    if objective == "mc_then_mape":
        ...
        if bool(sweep_exact):
            exact_recs: list[dict[str, Any]] = []
            for h_cfg, cfg in unique_cfgs.items():
                try:
                    full_rec = _train_eval_photonic_config(
                        kind=kind,
                        seed=int(seed),
                        model_kwargs=dict(cfg),
                        x_train=x_train,
                    ...
                    full_rec["phase"] = "sweep_exact_full"
                    full_rec["objective"] = str(objective)
                    full_rec["objective_components"] = json.dumps(
                        {
                            "objective_name": str(full_rec.get("objective_name", objective)),
                            "objective_value": float(full_rec.get("objective_value", np.nan)),
                            "val_surface_mape": float(full_rec.get("val_surface_mape", np.nan)),
                            "val_surface_wape": float(full_rec.get("val_surface_wape", np.nan)),
                            "val_surface_mae": float(full_rec.get("val_surface_mae", np.nan)),
                            "val_surface_rmse": float(full_rec.get("val_surface_rmse", np.nan)),
                            "residual_penalty": float(full_rec.get("residual_penalty", np.nan)),
                            "score": float(full_rec.get("score", np.nan)),
                        },
                        sort_keys=True,
                        separators=(",", ":"),
                    )
                    exact_recs.append(full_rec)
                    history.append(
                        {
                            **{k: v for k, v in full_rec.items() if k != "model_obj"},
                            "model_hash": str(full_rec.get("model_hash", h_cfg)),
                        }
                    )
                except Exception as exc:
                    history.append(
                        {
                            "phase": "sweep_exact_full",
                            "model_hash": str(h_cfg),
                            "model_kwargs": dict(cfg),
                            "failed": True,
                            "error": str(exc),
                            "objective": str(objective),
            ...
            if not exact_recs:
                raise RuntimeError(f"{kind}: exact full sweep failed for every candidate config.")
            exact_sorted = sorted(
                exact_recs,
                key=lambda r: (
            ...
            top_full = exact_sorted[: int(max(1, topk))]
            top_cfgs = [{"model_kwargs": dict(r.get("model_kwargs", {})), "model_hash": str(r.get("model_hash", ""))} for r in top_full]
        else:
    ...
    if not (objective != "mc_then_mape" and bool(sweep_exact)):
        top_full = []
    ...
    for rank, rec in enumerate(top_full, start=1):
        rec["sweep_rank"] = int(rank)
    ...
    best = top_full[0]
    if bool(sweep_replay_check):
        replay = _train_eval_photonic_config(
            kind=kind,
            seed=int(seed),
            model_kwargs=dict(best.get("model_kwargs", {})),
            x_train=x_train,
            y_train=y_train,
            x_val=x_val,
            y_val=y_val,
        ...
        d_obj = float(abs(float(replay.get("objective_value", np.nan)) - float(best.get("objective_value", np.nan))))
        d_map = float(abs(float(replay.get("val_mape", np.nan)) - float(best.get("val_mape", np.nan))))
        if (not np.isfinite(d_obj)) or (not np.isfinite(d_map)) or d_obj > 1e-10 or d_map > 1e-10:
            raise RuntimeError(
                f"{kind}: sweep replay check failed for selected best config "
                f"(delta_objective={d_obj:.3e}, delta_val_mape={d_map:.3e})."
            )
        history.append(
            {
                "phase": "best_replay_check",
                "model_hash": str(replay.get("model_hash", best.get("model_hash", ""))),
                "model_kwargs": dict(replay.get("model_kwargs", best.get("model_kwargs", {}))),
                "failed": False,
                "collapsed": bool(replay.get("collapsed", False)),
                "val_mape": float(replay.get("val_mape", np.nan)),
                "val_mae": float(replay.get("val_mae", np.nan)),
                "objective_name": str(replay.get("objective_name", objective)),
                "objective_value": float(replay.get("objective_value", np.nan)),
                "score": float(replay.get("score", np.nan)),
                "objective": str(objective),
                "objective_components": json.dumps(
                    {
                        "replay_check": True,
                        "delta_objective": float(d_obj),
                        "delta_val_mape": float(d_map),
                    },
                    sort_keys=True,
                    separators=(",", ":"),
                ),
...
def _build_quantum_comparison(
    ...
    q_res_tune = q_model.predict_residual(q_val_tune)
    q_res_val = q_model.predict_residual(q_val_eval)
    q_res_test = q_model.predict_residual(q_test)

    # Optimize blend weight on short-horizon validation (up to day-6), since this
    # workflow is primarily evaluated on near-term performance.
    h_opt = int(max(1, min(6, y_true_tune.shape[1], y_base_tune.shape[1], q_res_tune.shape[1])))
    y_true_tune_h = np.asarray(y_true_tune[:, :h_opt, :], dtype=float)
    y_base_tune_h = np.asarray(y_base_tune[:, :h_opt, :], dtype=float)
    q_res_tune_h = np.asarray(q_res_tune[:, :h_opt, :], dtype=float)
    y_true_tune_surface = _to_surface_space(bundle, y_true_tune_h)

    w_floor = float(np.clip(w_min, 0.0, 1.0))
    w_grid = sorted(
        {
            float(np.clip(w, 0.0, 1.0))
            for w in [0.0, w_floor, 0.05, 0.10, 0.15, 0.20, 0.30, 0.40, 0.50, 0.65, 0.80, 1.0]
        }
    )
    best_w = 0.0
    best_obj = float("inf")
    base_obj = float("inf")
    for w in w_grid:
        y_pred_tune_h = y_base_tune_h + float(w) * q_res_tune_h
        y_pred_tune_surface = _to_surface_space(bundle, y_pred_tune_h)
        obj = float(_compute_mape(y_true_tune_surface, y_pred_tune_surface, eps=float(mape_eps)))
        if abs(float(w)) <= 1e-12 and np.isfinite(obj):
            base_obj = float(obj)
        if np.isfinite(obj) and (obj < best_obj - 1e-12):
            best_obj = float(obj)
            best_w = float(w)
    # Conservative anti-overfit guard on the internal tune split:
    # keep quantum residual weight only when the tune gain is material.
    if np.isfinite(base_obj) and np.isfinite(best_obj):
        min_gain = float(max(0.15, 0.03 * base_obj))
        if (base_obj - best_obj) < min_gain:
            best_w = 0.0
            best_obj = float(base_obj)
    if train_fit_slice is not None and best_w > 0.0:
        best_w = float(np.clip(0.6 * best_w, 0.0, 1.0))
    if not np.isfinite(best_obj):
        best_w = float(w_floor)
        best_obj = float("nan")

    y_pred_train = y_base_train + float(best_w) * q_res_train
    y_pred_val = y_base_val + float(best_w) * q_res_val
    y_pred_test = y_base_test + float(best_w) * q_res_test
    floor = float(getattr(bundle, "surface_floor", 0.0))
    ...
    return {
        ...
        "w_star": float(best_w),
        "w_min": float(w_min),
        "w_star_tune_surface_mape_h6": float(best_obj),
        "w_star_tune_horizon": int(h_opt),
    ...
    for row in eval_rows:
        ...
        record = {
            ...
            "hashes": {
                ...
                "hash_state_features": None if q_features is None else stable_hash_array(np.asarray(q_features), name=f"{model}:state_features"),
                "hash_K_train": None if kernel_gram is None else stable_hash_array(np.asarray(kernel_gram), name=f"{model}:K_train"),
...
def run_comparison(
    ...
    classical_baseline_cache: str | Path | None = None,
    reuse_classical_baseline: bool = False,
    pqrc_M: int = 8,
    ...
    qk_gamma: float = 0.3,
    qk_gamma_grid: str = "",
    qk_ridge: float | None = None,
    qk_ridge_grid: str = "",
    qk_state_dim_mode: str = "mean_last_std",
    qk_feature: str = "clickprob",
    qk_shots: int = 32,
    qk_gain: float | None = None,
    qk_modes: int = 8,
    qk_in_scale: float = 0.5,
    qk_seed: int | None = None,
    qk_aux_blend: bool = True,
    qk_aux_kind: str = "photonic_qrc_feedback",
    qk_blend_objective: str = "mape",
    qk_blend_max_weight: float = 1.0,
    qk_blend_grid_points: int = 31,
    qk_horizon_mode: str = "single14",
    qk_target_space: str = "factor",
    qk_resid_clip: float | None = None,
    ...
    allow_stale_qrc_sweep: bool = False,
    ...
    pqrc_sweep_exact: bool = True,
    pqrc_sweep_replay_check: bool = True,
    sweep_modes: bool = False,
    sweep_gain: bool = False,
    sweep_input_scale: bool = False,
    sweep_ridge: bool = False,
    ...
    nn_train_loss_eff = str(nn_train_loss).strip().lower()
    if nn_train_loss_eff not in {"mae", "mse", "rmse"}:
    ...
    loaded_qrc_dataset_signature: str | None = None
    if qrc_load_from:
        ...
        loaded_cfg_dict, loaded_qrc_hash = _load_qrc_config_from_source(
            ...
            allow_stale_sweep=bool(allow_stale_qrc_sweep),
            preferred_models=preferred_qrc_models,
        )
        loaded_qrc_dataset_signature = (
            str(loaded_cfg_dict.get("dataset_signature", "")).strip() or None
        )
    ...
    qrc_code_signature = _qrc_code_signature()
    ...
    if loaded_qrc_dataset_signature and loaded_qrc_dataset_signature != dataset_signature:
        msg = (
            "[model_compare] qrc_load_from artifact dataset signature mismatch "
            f"(saved={loaded_qrc_dataset_signature}, current={dataset_signature}). "
            "Re-run pqrc_sweep on this exact dataset/split before replaying this config."
        )
        if bool(allow_stale_qrc_sweep):
            warnings.warn(msg + " Proceeding because --allow_stale_qrc_sweep is set.", RuntimeWarning)
        else:
            raise RuntimeError(msg + " Pass --allow_stale_qrc_sweep to override.")
    ...
    pqrc_modes_eff = int(np.clip(int(qrc_cfg.pqrc_modes), 1, 20))
    qk_horizon_mode_eff = str(qk_horizon_mode).strip().lower()
    if qk_horizon_mode_eff not in {"single14", "all"}:
        raise ValueError("--qk_horizon_mode must be one of: single14, all.")
    qk_target_space_eff = str(qk_target_space).strip().lower()
    if qk_target_space_eff not in {"factor", "surface"}:
        raise ValueError("--qk_target_space must be one of: factor, surface.")
    qk_state_dim_mode_eff = str(qk_state_dim_mode).strip().lower()
    if qk_state_dim_mode_eff not in {"mean_last", "mean_last_std"}:
        raise ValueError("--qk_state_dim_mode must be one of: mean_last, mean_last_std.")
    qk_feature_eff = str(qk_feature).strip().lower()
    if qk_feature_eff == "pseudo_coincidence":
        qk_feature_eff = "coincidence"
    if qk_feature_eff not in {"clickprob", "coincidence"}:
        raise ValueError("--qk_feature must be one of: clickprob, coincidence.")
    qk_gamma_eff = float(max(1e-12, float(qk_gamma)))
    qk_ridge_eff = float(max(1e-12, float(pqrc_ridge if qk_ridge is None else qk_ridge)))
    qk_gain_eff = float(max(0.0, float(pqrc_gain if qk_gain is None else qk_gain)))
    qk_modes_eff = int(max(2, int(qk_modes)))
    qk_in_scale_eff = float(max(1e-8, float(qk_in_scale)))
    qk_seed_eff = int(qrc_seed_eff if qk_seed is None else qk_seed)
    qk_aux_blend_eff = bool(qk_aux_blend)
    qk_aux_kind_eff = str(qk_aux_kind).strip().lower()
    ...
    qk_blend_objective_eff = str(qk_blend_objective).strip().lower()
    if qk_blend_objective_eff not in {"rmse", "mape"}:
        raise ValueError("--qk_blend_objective must be one of: rmse, mape.")
    qk_blend_max_weight_eff = float(np.clip(float(qk_blend_max_weight), 0.1, 3.0))
    qk_blend_grid_points_eff = int(np.clip(int(qk_blend_grid_points), 3, 81))
    qk_shots_eff = int(max(0, int(qk_shots)))
    qk_gamma_grid_eff = str(qk_gamma_grid or "").strip()
    qk_ridge_grid_eff = str(qk_ridge_grid or "").strip()
    run_config_hash = stable_hash_config(
        {
            ...
            "classical_baseline_cache": (None if classical_baseline_cache is None else str(classical_baseline_cache)),
            "reuse_classical_baseline": bool(reuse_classical_baseline),
            ...
            "qk_gamma": float(qk_gamma_eff),
            "qk_gamma_grid": str(qk_gamma_grid_eff),
            "qk_ridge": float(qk_ridge_eff),
            "qk_ridge_grid": str(qk_ridge_grid_eff),
            "qk_state_dim_mode": str(qk_state_dim_mode_eff),
            "qk_feature": str(qk_feature_eff),
            "qk_shots": int(qk_shots_eff),
            "qk_gain": float(qk_gain_eff),
            "qk_modes": int(qk_modes_eff),
            "qk_in_scale": float(qk_in_scale_eff),
            "qk_seed": int(qk_seed_eff),
            "qk_aux_blend": bool(qk_aux_blend_eff),
            "qk_aux_kind": str(qk_aux_kind_eff),
            "qk_blend_objective": str(qk_blend_objective_eff),
            "qk_blend_max_weight": float(qk_blend_max_weight_eff),
            "qk_blend_grid_points": int(qk_blend_grid_points_eff),
            "qk_horizon_mode": str(qk_horizon_mode_eff),
            "qk_target_space": str(qk_target_space_eff),
            "qk_resid_clip": None if qk_resid_clip is None else float(qk_resid_clip),
            "qrc_mode": str(qrc_mode),
            "qrc_target": str(qrc_target),
            "qrc_baseline": str(qrc_baseline),
            "target_transform": str(target_transform),
            ...
            "pqrc_sweep_exact": bool(pqrc_sweep_exact),
            "pqrc_sweep_replay_check": bool(pqrc_sweep_replay_check),
            ...
            "allow_stale_qrc_sweep": bool(allow_stale_qrc_sweep),
            "qrc_config_hash": str(qrc_config_hash),
            "qrc_code_signature": str(qrc_code_signature),
    ...
    classical_models_all: list[tuple[str, str]] = [
        ...
        ("lstm", "lstm"),
        ("reservoir", "reservoir"),
        ("hybrid_qrc_fast", "hybrid_qrc_fast"),
        ("qml_classical_reservoir", "qml_classical_reservoir"),
        ("qml_recursive_classical_reservoir", "qml_recursive_classical_reservoir"),
        ("qml_pca_lstm", "qml_pca_lstm"),
        ("qml_convlstm", "qml_convlstm"),
        ...
        ("quantum_kernel_photonic_memory", "quantum_kernel_photonic_memory"),
    ...
    cached_eval_rows_by_model: dict[str, dict[str, Any]] = {}
    reused_classical_models: set[str] = set()
    classical_cache_path = (
        Path(str(classical_baseline_cache)).expanduser()
        if (classical_baseline_cache is not None and str(classical_baseline_cache).strip())
        else None
    )
    classical_code_signature = _classical_baseline_code_signature()
    classical_baseline_signature = stable_hash_config(
        {
            "dataset_signature": str(dataset_signature),
            "level": int(level),
            "lookback": int(L),
            ...
            "qk_target_space": str(qk_target_space_eff),
            "qk_resid_clip": (None if qk_resid_clip is None else float(qk_resid_clip)),
            "nn_train_loss": str(nn_train_loss_eff),
            "qrc_mode": str(qrc_mode),
            "qrc_target": str(qrc_target),
            "qrc_baseline": str(qrc_baseline),
            "target_transform": str(target_transform),
    ...
    required_classical_for_run = {str(name).strip().lower() for name, _ in classical_models}
    if bool(reuse_classical_baseline) and classical_cache_path is not None:
        payload = _load_classical_baseline_cache(classical_cache_path)
        if payload is None:
            print(f"[model_compare] classical baseline cache not available: {classical_cache_path}")
        else:
            ...
            if not isinstance(meta, dict):
                ...
                meta = {}
            if str(meta.get("dataset_signature", "")) != str(dataset_signature):
                reasons.append("dataset_signature")
            if int(meta.get("level", -1)) != int(level):
                reasons.append("level")
            if int(meta.get("lookback", -1)) != int(L):
                reasons.append("lookback")
            if int(meta.get("horizon", -1)) != int(H):
                reasons.append("horizon")
            if str(meta.get("classical_code_signature", "")) != str(classical_code_signature):
                reasons.append("classical_code_signature")
            if str(meta.get("classical_baseline_signature", "")) != str(classical_baseline_signature):
                reasons.append("classical_baseline_signature")
            if reasons:
                print(
                    f"[model_compare] classical baseline cache mismatch at {classical_cache_path}; "
                    f"ignoring cache (reasons={reasons})."
                )
            else:
                stored_blob = payload.get("stored_classical", {})
                rows_blob = payload.get("eval_rows", {})
                if isinstance(stored_blob, dict):
                    for model_name in sorted(required_classical_for_run):
                        blk = _copy_stored_prediction_block(stored_blob.get(model_name))
                        if blk is None:
                            continue
                        stored_classical[model_name] = blk
                        reused_classical_models.add(model_name)
                if include_classical and isinstance(rows_blob, dict):
                    for model_name in sorted(reused_classical_models):
                        row_obj = rows_blob.get(model_name)
                        if isinstance(row_obj, dict):
                            cached_eval_rows_by_model[model_name] = copy.deepcopy(row_obj)
                print(
                    f"[model_compare] reused cached classical baseline models={sorted(reused_classical_models)} "
                    f"from {classical_cache_path}"
                )
    ...
    for model_name, kind in classical_models:
        if model_name in reused_classical_models:
            if include_classical:
                cached_row = cached_eval_rows_by_model.get(model_name)
                if isinstance(cached_row, dict):
                    eval_rows.append(copy.deepcopy(cached_row))
                else:
                    cached_pred = stored_classical[model_name]
                    y_train_cached = np.asarray(cached_pred["train"], dtype=float)
                    y_val_cached = np.asarray(cached_pred["val"], dtype=float)
                    y_val_surface_cached = np.asarray(cached_pred["val_surface"], dtype=float)
                    y_train_surface_cached = _to_surface_space(bundle, y_train_cached)
                    split_extra_cached = _split_surface_mape_extra(
                        y_true_train_surface=y_true_train_surface,
                        y_pred_train_surface=y_train_surface_cached,
                        y_true_val_surface=y_true_val_surface,
                        y_pred_val_surface=y_val_surface_cached,
                        eps=float(mape_eps),
                    )
                    eval_rows.append(
                        _evaluate_model(
                            name=model_name,
                            y_pred_test_norm=y_val_cached,
                            y_true_test_norm=y_true_eval_norm,
                            y_true_test_surface=y_true_eval_surface,
                            bundle=bundle,
                            mape_eps=mape_eps,
                            y_pred_test_surface=y_val_surface_cached,
                            extra={"features_input": bundle.x_val_norm, **split_extra_cached},
                        )
                    )
            print(f"[model_compare] reused classical baseline for {model_name}; skipping retrain.")
            continue
        kind_norm = str(kind).strip().lower()
        if kind_norm == "quantum_kernel_photonic_memory" and not bool(include_quantum):
            print("[model_compare] skipping quantum_kernel_photonic_memory because --skip_quantum is active.")
            continue
        if kind_norm in {
            "reservoir",
            "hybrid_qrc_fast",
        ...
        elif kind_norm == "quantum_kernel_photonic_memory":
            model_kwargs = {
                "qk_gamma": float(qk_gamma_eff),
                "qk_gamma_grid": str(qk_gamma_grid_eff),
                "qk_ridge": float(qk_ridge_eff),
        ...
        elif kind_norm == "hybrid_qrc_fast":
            model_kwargs = {
                "hybrid_qrc_modes": int(pqrc_modes_eff),
                "hybrid_qrc_shots": int(pqrc_shots_eff),
                "hybrid_qrc_input_scale": float(pqrc_input_scale),
                "hybrid_qrc_ridge": float(pqrc_ridge),
                "hybrid_qrc_feature_mode": "mean_last_std",
                "hybrid_qrc_use_cycle_phase": bool(use_cycle_phase),
                "hybrid_qrc_use_dt": True,
                "hybrid_qrc_cycle_period": 19,
                "hybrid_qrc_state_dim": int(max(32, 8 * int(pqrc_modes_eff))),
                "hybrid_qrc_quantum_rank": int(max(4, min(16, int(pqrc_modes_eff)))),
                "hybrid_qrc_fast_mode": True,
                "hybrid_qrc_residual_clip": (None if qrc_resid_clip is None else float(qrc_resid_clip)),
                "nn_train_loss": str(nn_train_loss_eff),
                "seed": int(model_seed),
                "level": int(level),
                "lookback": int(L),
        ...
        elif kind_norm in {"mlp", "gru", "lstm", "qml_pca_lstm", "qml_convlstm"}:
            model_kwargs = {
                "nn_train_loss": str(nn_train_loss_eff),
            }
        ...
        if kind_norm in {
            ...
            sweep_res = _run_photonic_coordinate_sweep(
                ...
                sweep_input_scale=bool(sweep_input_scale),
                sweep_ridge=bool(sweep_ridge),
                sweep_exact=bool(pqrc_sweep_exact),
                sweep_replay_check=bool(pqrc_sweep_replay_check),
            )
            best = sweep_res["best"]
        ...
        if kind_norm in {
            ...
            if isinstance(fstats, dict):
                ...
                if "wedge_indices" in fstats:
                    model_kwargs["wedge_indices"] = [int(v) for v in list(fstats.get("wedge_indices", []))]
                if "gamma_selected" in fstats:
                    model_kwargs["qk_gamma"] = float(fstats.get("gamma_selected", np.nan))
                if "gamma_grid" in fstats:
                    gg = fstats.get("gamma_grid", [])
                    model_kwargs["qk_gamma_grid"] = ",".join(str(float(v)) for v in list(gg))
                for k in (
                    "pqrc_pseudocount",
                    "min_prob_before",
                    "min_prob_after",
            ...
            k_train_arr = getattr(model, "kernel_gram_", None)
            if k_train_arr is not None:
                model_kwargs["kernel_gram"] = np.asarray(k_train_arr, dtype=float)
            state_hash = getattr(model, "state_hash", None)
            if state_hash is not None:
                model_kwargs["hash_state_features"] = str(state_hash)
            kernel_hash = getattr(model, "kernel_hash", None)
            if kernel_hash is not None:
                model_kwargs["hash_K_train"] = str(kernel_hash)
            if hasattr(model, "n_train_kernel"):
                model_kwargs["n_train_kernel"] = int(getattr(model, "n_train_kernel"))
            if hasattr(model, "kernel_train_rows"):
                model_kwargs["kernel_train_rows"] = int(getattr(model, "kernel_train_rows"))
            if hasattr(model, "kernel_train_cols"):
                model_kwargs["kernel_train_cols"] = int(getattr(model, "kernel_train_cols"))
            if hasattr(model, "objective_train_rmse"):
                model_kwargs["qk_objective_train_rmse"] = float(getattr(model, "objective_train_rmse"))
            if hasattr(model, "objective_val_rmse"):
                model_kwargs["qk_objective_val_rmse"] = float(getattr(model, "objective_val_rmse"))
            if hasattr(model, "selected_horizons"):
                model_kwargs["qk_selected_horizons"] = [
                    int(v) + 1 for v in list(getattr(model, "selected_horizons"))
                ]
            if hasattr(model, "qk_horizon_mode"):
                model_kwargs["qk_horizon_mode"] = str(getattr(model, "qk_horizon_mode"))
            if hasattr(model, "qk_target_space"):
                model_kwargs["qk_target_space"] = str(getattr(model, "qk_target_space"))
            qk_params = getattr(model, "params", None)
            if qk_params is not None:
                if hasattr(qk_params, "gamma"):
                    model_kwargs["qk_gamma"] = float(getattr(qk_params, "gamma"))
                if hasattr(qk_params, "gamma_grid"):
                    gg = getattr(qk_params, "gamma_grid")
                    if isinstance(gg, (list, tuple)):
                        model_kwargs["qk_gamma_grid"] = ",".join(str(float(v)) for v in gg)
                if hasattr(qk_params, "aux_blend"):
                    model_kwargs["qk_aux_blend"] = bool(getattr(qk_params, "aux_blend"))
                if hasattr(qk_params, "aux_kind"):
                    model_kwargs["qk_aux_kind"] = str(getattr(qk_params, "aux_kind"))
                if hasattr(qk_params, "blend_objective"):
                    model_kwargs["qk_blend_objective"] = str(getattr(qk_params, "blend_objective"))
                if hasattr(qk_params, "blend_max_weight"):
                    model_kwargs["qk_blend_max_weight"] = float(getattr(qk_params, "blend_max_weight"))
                if hasattr(qk_params, "blend_grid_points"):
                    model_kwargs["qk_blend_grid_points"] = int(getattr(qk_params, "blend_grid_points"))
    ...
    if classical_cache_path is not None and stored_classical:
        cache_rows: dict[str, dict[str, Any]] = {}
        for row in eval_rows:
            model_row = str(row.get("model", "")).strip().lower()
            if model_row in stored_classical and _model_family(model_row) in {"persistence", "classical"}:
                cache_rows[model_row] = copy.deepcopy(row)
        cache_payload = {
            "meta": {
                "dataset_signature": str(dataset_signature),
                "level": int(level),
                "lookback": int(L),
                "horizon": int(H),
                "classical_code_signature": str(classical_code_signature),
                "classical_baseline_signature": str(classical_baseline_signature),
                "qrc_code_signature": str(qrc_code_signature),
                "run_config_hash": str(run_config_hash),
                "models": sorted(stored_classical.keys()),
                "created_utc": pd.Timestamp.utcnow().isoformat(),
            },
            "stored_classical": stored_classical,
            "eval_rows": cache_rows,
        }
        try:
            _save_classical_baseline_cache(classical_cache_path, cache_payload)
            print(
                f"[model_compare] wrote classical baseline cache models={sorted(stored_classical.keys())} "
                f"to {classical_cache_path}"
            )
        except Exception as exc:
            warnings.warn(f"failed to write classical baseline cache to {classical_cache_path}: {exc}", RuntimeWarning)
        ...
        metrics_rows.append(
            {
                ...
                "qk_gamma": row.get("qk_gamma", np.nan),
                "qk_gamma_grid": row.get("qk_gamma_grid"),
                "qk_ridge": row.get("qk_ridge", np.nan),
                "qk_ridge_grid": row.get("qk_ridge_grid"),
                "qk_state_dim_mode": row.get("qk_state_dim_mode"),
                "qk_feature": row.get("qk_feature"),
                "qk_shots": row.get("qk_shots", np.nan),
                "qk_gain": row.get("qk_gain", np.nan),
                "qk_modes": row.get("qk_modes", np.nan),
                "qk_in_scale": row.get("qk_in_scale", np.nan),
                "qk_seed": row.get("qk_seed", np.nan),
                "qk_aux_blend": row.get("qk_aux_blend"),
                "qk_aux_kind": row.get("qk_aux_kind"),
                "qk_blend_objective": row.get("qk_blend_objective"),
                "qk_blend_max_weight": row.get("qk_blend_max_weight", np.nan),
                "qk_blend_grid_points": row.get("qk_blend_grid_points", np.nan),
                "qk_resid_clip": row.get("qk_resid_clip", np.nan),
                "qk_horizon_mode": row.get("qk_horizon_mode"),
                "qk_target_space": row.get("qk_target_space"),
                "qk_selected_horizons": json.dumps([int(v) for v in (row.get("qk_selected_horizons") or [])], separators=(",", ":")),
                "qk_objective_train_rmse": row.get("qk_objective_train_rmse", np.nan),
                "qk_objective_val_rmse": row.get("qk_objective_val_rmse", np.nan),
                "n_train_kernel": row.get("n_train_kernel", np.nan),
                "kernel_train_rows": row.get("kernel_train_rows", np.nan),
                "kernel_train_cols": row.get("kernel_train_cols", np.nan),
                "hash_state_features": row.get("hash_state_features"),
                "hash_K_train": row.get("hash_K_train"),
                "wedge_indices": json.dumps([int(v) for v in (row.get("wedge_indices") or [])], separators=(",", ":")),
    ...
    if best_qrc_config_to_save is not None:
        best_cfg_dict = QRCConfig.from_dict(best_qrc_config_to_save).validate().to_dict()
        best_cfg_dict["dataset_signature"] = str(dataset_signature)
        best_cfg_dict["run_config_hash"] = str(run_config_hash)
        best_cfg_dict["qrc_code_signature"] = str(qrc_code_signature)
        save_config(best_cfg_dict, out_dir_p / "best_config.json")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data_dir", type=str, default="Quandela/Challenge_Swaptions_from_zip")
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "hybrid_qrc_fast,"
            "qml_classical_reservoir,qml_recursive_classical_reservoir,qml_pca_lstm,qml_convlstm,"
            "photonic_qrc_feedback,photonic_qrc_no_feedback,photonic_memory,photonic_memory_no_feedback,"
            "quantum_kernel_photonic_memory,persist_qrc_weak"
        ),
        help=(
    ...
    parser.add_argument("--models", type=str, default="", help="Alias for --classical_models.")
    parser.add_argument(
        "--classical_baseline_cache",
        type=str,
        default="",
        help="Optional pickle path for stored classical predictions/rows reused across quantum reruns.",
    )
    parser.add_argument(
        "--reuse_classical_baseline",
        action="store_true",
        help="Reuse cached classical baseline from --classical_baseline_cache when signatures match.",
    )
    ...
    parser.add_argument(
        ...
        default="mae",
        help="Training loss for torch sequence models (mlp/gru/lstm/qml_pca_lstm/qml_convlstm).",
    )
    ...
    parser.add_argument("--qk_gamma", type=float, default=0.3)
    parser.add_argument("--qk_gamma_grid", type=str, default="", help="Comma-separated gamma grid for qk kernel-width selection.")
    parser.add_argument("--qk_ridge", type=float, default=None)
    parser.add_argument("--qk_ridge_grid", type=str, default="", help="Comma-separated ridge grid for qk lambda selection.")
    parser.add_argument("--qk_state_dim_mode", type=str, choices=["mean_last", "mean_last_std"], default="mean_last_std")
    parser.add_argument("--qk_feature", type=str, choices=["clickprob", "coincidence"], default="clickprob")
    parser.add_argument("--qk_shots", type=int, default=32)
    parser.add_argument("--qk_gain", type=float, default=None)
    parser.add_argument("--qk_modes", type=int, default=8)
    parser.add_argument("--qk_in_scale", type=float, default=0.5)
    parser.add_argument("--qk_seed", type=int, default=None)
    parser.add_argument(
        "--qk_aux_blend",
        type=int,
        choices=[0, 1],
        default=1,
        help="Enable (1) or disable (0) auxiliary residual blend for quantum kernel model.",
    )
    parser.add_argument(
        "--qk_aux_kind",
        type=str,
        choices=["photonic_qrc_feedback", "reservoir", "none"],
        default="photonic_qrc_feedback",
        help="Auxiliary model kind used for optional blend.",
    )
    parser.add_argument(
        "--qk_blend_objective",
        type=str,
        choices=["rmse", "mape"],
        default="mape",
        help="Validation objective used to select qk/aux blend weights.",
    )
    parser.add_argument(
        "--qk_blend_max_weight",
        type=float,
        default=1.0,
        help="Maximum nonnegative blend weight searched for qk/aux contributions.",
    )
    parser.add_argument(
        "--qk_blend_grid_points",
        type=int,
        default=31,
        help="Number of grid points per source weight for blend search.",
    )
    parser.add_argument("--qk_horizon_mode", type=str, choices=["single14", "all"], default="single14")
    parser.add_argument("--qk_target_space", type=str, choices=["factor", "surface"], default="factor")
    parser.add_argument("--qk_resid_clip", type=float, default=None)
    ...
    parser.add_argument(
        "--pqrc_sweep_exact",
        dest="pqrc_sweep_exact",
        action="store_true",
        default=True,
        help="Re-evaluate all swept candidate configs on full train/tune data and select the true best under the sweep objective.",
    )
    parser.add_argument(
        "--no_pqrc_sweep_exact",
        dest="pqrc_sweep_exact",
        action="store_false",
        help="Disable exact full re-evaluation of all swept candidates (faster, less reliable).",
    )
    parser.add_argument(
        "--pqrc_sweep_replay_check",
        dest="pqrc_sweep_replay_check",
        action="store_true",
        default=True,
        help="Replay selected best swept config once more and fail if objective/val_mape drift is detected.",
    )
    parser.add_argument(
        "--no_pqrc_sweep_replay_check",
        dest="pqrc_sweep_replay_check",
        action="store_false",
        help="Disable replay consistency check for selected sweep winner.",
    )
    ...
    parser.add_argument(
        "--allow_stale_qrc_sweep",
        action="store_true",
        help="Allow replaying qrc_load_from artifacts generated under a different QRC code signature.",
    )
...
def main() -> None:
    ...
    classical_models_raw = str(args.models).strip() if str(args.models).strip() else str(args.classical_models)
    classical_models = [t.strip().lower() for t in classical_models_raw.split(",") if t.strip()]
    ...
    metrics_df, out_dir_p = run_comparison(
        ...
        classical_baseline_cache=(str(args.classical_baseline_cache).strip() or None),
        reuse_classical_baseline=bool(args.reuse_classical_baseline),
        fast=bool(args.fast),
        ...
        nn_train_loss=str(args.nn_train_loss),
        ...
        qk_gamma=float(args.qk_gamma),
        qk_gamma_grid=str(args.qk_gamma_grid),
        qk_ridge=(None if args.qk_ridge is None else float(args.qk_ridge)),
        qk_ridge_grid=str(args.qk_ridge_grid),
        qk_state_dim_mode=str(args.qk_state_dim_mode),
        qk_feature=str(args.qk_feature),
        qk_shots=int(args.qk_shots),
        qk_gain=(None if args.qk_gain is None else float(args.qk_gain)),
        qk_modes=int(args.qk_modes),
        qk_in_scale=float(args.qk_in_scale),
        qk_seed=(None if args.qk_seed is None else int(args.qk_seed)),
        qk_aux_blend=bool(int(args.qk_aux_blend)),
        qk_aux_kind=str(args.qk_aux_kind),
        qk_blend_objective=str(args.qk_blend_objective),
        qk_blend_max_weight=float(args.qk_blend_max_weight),
        qk_blend_grid_points=int(args.qk_blend_grid_points),
        qk_horizon_mode=str(args.qk_horizon_mode),
        qk_target_space=str(args.qk_target_space),
        qk_resid_clip=(None if args.qk_resid_clip is None else float(args.qk_resid_clip)),
        ...
        allow_stale_qrc_sweep=bool(args.allow_stale_qrc_sweep),
        qrc_sweep_objective=str(sweep_objective_eff),
        sweep_objective=str(sweep_objective_eff),
        pqrc_sweep=bool(args.pqrc_sweep),
        pqrc_sweep_exact=bool(args.pqrc_sweep_exact),
        pqrc_sweep_replay_check=bool(args.pqrc_sweep_replay_check),
