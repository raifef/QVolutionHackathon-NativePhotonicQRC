...
@dataclass
class PhotonicQRCDisabledForecaster:
    y_dim: int
    horizon: int
    d_factors: int
    params: PhotonicQRCParams
    qrc_mode: str = "disabled"
    ...
    def _baseline_from_x(self, x_seq: np.ndarray) -> np.ndarray:
        ...
        if x.shape[2] < int(self.y_dim):
            raise ValueError(f"x_seq feature dim {x.shape[2]} must be >= target dim {self.y_dim}.")
        return np.repeat(x[:, -1:, : int(self.y_dim)], int(self.horizon), axis=1)

    def predict(self, x_seq: np.ndarray) -> np.ndarray:
        x = np.asarray(x_seq, dtype=np.float64)
        if x.ndim != 3:
            raise ValueError(f"x_seq must be [N,L,D], got {x.shape}.")
        y_hat = self._baseline_from_x(x)
        self.last_q_features_ = np.zeros((int(x.shape[0]), 0), dtype=np.float64)
        self.last_gate_ = np.zeros_like(y_hat, dtype=np.float64)
        return y_hat
    ...
    def run_feedback_sanity(self, x_seq: np.ndarray, max_n: int = 64) -> dict[str, Any]:
        ...
        return {
            ...
            "pqrc_gain": 0.0,
            "checksum_gain0": "disabled",
            "checksum_gain1": "disabled",
...
def train_photonic_qrc_forecaster(
    ...
    *,
    ...
    warnings.warn(
        "photonic_qrc is disabled in this build; using fast persistence fallback.",
        RuntimeWarning,
    )
    model_disabled = PhotonicQRCDisabledForecaster(
        y_dim=int(ytr.shape[2]),
        horizon=int(ytr.shape[1]),
        d_factors=int(min(ytr.shape[2], xtr.shape[2])),
        params=params,
        qrc_mode="disabled",
        qrc_target=str(params.qrc_target),
        ...
        residual_clip=0.0,
        tau=0.0,
        mode_val_mape={"disabled": float(np.mean(np.abs(yva - np.repeat(xva[:, -1:, : ytr.shape[2]], ytr.shape[1], axis=1))))},
        feature_stats={
            "disabled": True,
            "reason": "photonic_qrc_ignored",
            "effective_model": "persistence",
        },
        train_qevals=0,
        train_total_shots=0,
        infer_qevals=0,
    ...
    model_disabled.feedback_sanity = model_disabled.run_feedback_sanity(xva, max_n=64)
